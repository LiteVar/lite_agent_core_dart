...
class JsonRPCHttpResponseBody {
  ...
  factory JsonRPCHttpResponseBody.fromJson(Map<String, dynamic> json) {
    return JsonRPCHttpResponseBody(
      ...
      error: json["error"] == null? null: JsonRPCHttpResponseBodyError.fromJson(json["error"]),
      id: json["id"]
    );
  }
...
Future<JsonRPCHttpResponse> requestJsonRPCHttpAPI(JsonRPCHttpRequest jsonRPCHttpRequest, {String? authorization}) async {
  ...
  if (httpAPIResponse.statusCode == 200) {
    ...
    if(jsonRPCHttpResponseBody.error != null) {
      return JsonRPCHttpResponse(statusCode: jsonRPCHttpResponseBody.error!.code, body: jsonRPCHttpResponseBody.error!.message);
    } else {
      return JsonRPCHttpResponse(statusCode: httpAPIResponse.statusCode, body: jsonEncode(jsonRPCHttpResponseBody.result!));
    }
  } else {
